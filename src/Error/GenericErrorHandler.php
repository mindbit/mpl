...
class GenericErrorHandler extends AbstractErrorHandler
{
     ...
     * outbut buffering.
     */
    public function getOutputBuffer($all = false)
    {
        $ret = "";
     ...
     * extension).
     */
    public function disableBuffering($all = true)
    {
        echo $this->getOutputBuffer($all);
    }

    public function displayErrorHeader()
    {
        switch ($this->displayFormat) {
            ...
            case self::DISPLAY_FORMAT_TEXT:
                ...
                echo AsciiTable::renderCells(
                    array('ERROR'),
                    array(EXC_TABLE_WIDTH + 3),
                    array(1),
                    array(STR_ALIGN_CENTER)
                );
    ...
    public function displayErrorBody($data)
    {
        switch ($this->displayFormat) {
            case self::DISPLAY_FORMAT_HTML:
                ?>
    ...
    public function displayErrorFooter()
    {
        switch ($this->displayFormat) {
            case self::DISPLAY_FORMAT_HTML:
                ?>
    ...
    public function formatErrorHeader()
    {
        return "============================== ERROR ==============================\n";
    }
    ...
    public function formatErrorFooter()
    {
        return "===================================================================\n";
    }
    ...
    public function mailErrorSubject($data)
    {
        return "[" . php_uname("n") . "] error in file " . $data["filename"] .
            ", line " . $data["line"];
    }

    public function logErrorHeader()
    {
        $this->logFormatted($this->formatErrorHeader());
    }

    public function logErrorBody($data)
    {
        $this->logFormatted($this->formatErrorBody($data));
    }

    public function logErrorFooter()
    {
    ...
    public function handleSingleError($data)
    {
        ...
        if ($this->mailProps) {
            mail(
                $this->mailProps[self::MAIL_TO],
                $this->mailErrorSubject($data),
