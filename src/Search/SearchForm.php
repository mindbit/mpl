...
abstract class SearchForm extends BaseForm
{
    ...
    public function displayNoResults()
    {
        ?>
    ...
    public function displayNavBarTop()
    {
        ?>
        <table width="100%">
        <tr>
            <td width="33%">
            Results <b><?= $this->offset + 1?></b> -
            <b><?= min($this->offset + $this->limit, $this->pager->getTotalRecordCount())?></b>
            out of <b><?= $this->pager->getTotalRecordCount()?></b></td>
            <td align="center"><?php $this->displayPageList(); ?></td>
            <td width="33%" align="right">Results/page
            <?= HTML::select(
                "__search_rpp",
                $this->limit,
                $this->getRppOptions(),
                false,
                null,
                array("onChange" => "searchChangeRpp()")
            )
            ?>
            </td>
        </tr>
    ...
    public function displayNavBarBottom()
    {
        ?>
        <hr width="100%">
        <table width="100%">
        <tr>
            ...
            <td width="33%" align="right">
                ...
                <?= $this->pager->getPage() < $this->pager->getTotalPages() ? "" : "disabled"?>
                onClick="searchPage(<?= $this->pager->getPage() + 1?>)">
            </td>
        </tr>
        </table>
        <?php
    ...
    public function displayResultsHeader()
    {
        echo '<table width="100%">';
    }

    public function displayResultsFooter()
    {
        echo "</table>";
    }

    abstract public function displayResult($result);
